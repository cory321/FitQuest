...
export const MonthView = memo(function MonthView({
	...
	// Calculate workout counts by date and track completion status
	...
	// Get heat map color based on workout count and completion status
	const getHeatMapColor = (count: number, isCompleted: boolean) => {
		if (count === 0) return 'transparent';
		const intensity = getWorkoutIntensity(count);
		const alpha = 0.2 + intensity * 0.6;
		
		// Use emerald green for completed workouts (better contrast), orange for incomplete
		if (isCompleted) {
			return `rgba(16, 185, 129, ${alpha})`; // emerald-500: rgb(16, 185, 129)
		}
			...
			{calendarDays.map((day) => {
				...
				return (
					<motion.button
						...
						`}
						style={{
							backgroundColor: getHeatMapColor(count, isCompleted),
					...
					>
						...
						{count > 0 && (
							<motion.span
								...
								className={`inline-flex items-center justify-center w-5 h-5 text-[10px] font-bold rounded-full mt-0.5 shadow-sm ${
									isCompleted
										? 'bg-emerald-600 dark:bg-lime-600 text-white'
